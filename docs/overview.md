...
- **First upload, no comparison**: The first submission for a (project, suite, name, browser, size) has no baseline; there's nothing to compare it against, so it's stored with no comparison images, passes automatically, and marks `is_new_baseline = true`. The SPA shows a "New baseline" chip so humans can spot auto-baselined keys.
