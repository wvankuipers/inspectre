...
| 3 | First-time baselining                               | **Auto self-baseline + UI badge.** Auto-baseline on first run for a new key (no friction for ingest), but show a "new baseline" badge on the run page so humans see it. |
...
**Decision #4 (auto-update slug) + Decision #3 (auto self-baseline) + the `key` formula** — renaming a project from "Acme" to "Acme Inc" changes every contained test's `key` (because `key` is `project.name` slugified). Every existing baseline becomes orphan; the next test for each key auto-self-baselines (so it passes). The user will see "new baseline" badges (decision #3) on every test in the next run. **This is the intended behaviour given the decisions, but it surprises admins.** Document it on the admin rename screen and in `admin.md`.
