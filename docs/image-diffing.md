...
The orchestrator. Reads from the saved `Test`, downloads the previous baseline (if any) from S3, runs the diff, uploads the three result files plus thumbnails, and upserts the `Baseline` row.
...
class ScreenshotComparison:
    ...
    def _stage_upload(self, tmp: Path) -> Path:
        """Save Django's UploadedFile to a real path so ImageMagick can read it."""
        dest = tmp / 'upload.png'
    ...
    def _crop_in_place(self, src: Path) -> None:
        ...
        if result.returncode != 0:
            logger.error("crop failed", extra={
                'test_id': self.test.id, 'stderr': result.stderr.strip(),
            })
    ...
    def _compare(self, canvas, screenshot_in, baseline_in, paths) -> int:
        ...
        compare_cmd = [
            "compare", "-alpha", "Off", "-dissimilarity-threshold", "1",
            "-fuzz", self.test.fuzz_level, "-metric", "AE",
            "-highlight-color", f"#{self.test.highlight_colour}",
            str(paths['baseline']), str(paths['screenshot']), str(paths['diff']),
        ]
        try:
            for cmd in pad_cmds:
            ...
            result = subprocess.run(compare_cmd, capture_output=True, text=True, check=False,
                                    timeout=settings.IMAGEMAGICK_TIMEOUT_SECONDS)
        except subprocess.TimeoutExpired as exc:
            raise ImageDiffError("ImageMagick timed out") from exc
        # `compare` exits 0 (identical) or 1 (different) — both fine. Anything ≥ 2 is broken.
        if result.returncode not in (0, 1):
            logger.error("ImageMagick compare failed", extra={
                'test_id': self.test.id, 'returncode': result.returncode,
                'stderr': result.stderr.strip(),
            })
            raise ImageDiffError(f"compare failed (rc={result.returncode}): {result.stderr.strip()}")

        # AE prints the pixel count to stderr.
        try:
            return int(result.stderr.strip().split()[0])
        except (ValueError, IndexError):
            raise ImageDiffError(f"could not parse compare output: {result.stderr!r}")

    def _record_result(self, canvas: Canvas, diff_path: Path, diff_pixels: int) -> None:
        """Compute diff% and pass/fail, mutate the Test in memory (saved by _persist_files)."""
        total_pixels = canvas.width * canvas.height
        diff_pct = (diff_pixels / total_pixels) * 100 if total_pixels else 0
        self.test.diff = round(diff_pct, 2)
        self.test.passed = diff_pct < settings.IMAGE_DIFF_THRESHOLD

    def _persist_files(self, paths: dict[str, Path]) -> None:
        """Attach the three result files to Test FileFields and save."""
    ...
    def _upsert_baseline(self, screenshot_path: Path) -> None:
        """Race-safe upsert: select_for_update inside a transaction (decisions.md, race fix)."""
        with transaction.atomic():
            baseline, _ = (
                Baseline.objects.select_for_update()
                                .get_or_create(key=self.test.key, defaults={
                                    'name':     self.test.name,
                                    'browser':  self.test.browser,
                                    'size':     self.test.size,
                                    'suite_id': self.test.run.suite_id,
                                    'test_id':  self.test.id,
                                })
            )
            with screenshot_path.open('rb') as fh:
                baseline.screenshot.save('screenshot.png', File(fh), save=False)
            baseline.test_id = self.test.id
            baseline.save()
```
...
- Identical inputs → `passed=True`, `diff=0`, no diff highlights, `is_new_baseline=False` (when a baseline exists) / `True` (when none does).
